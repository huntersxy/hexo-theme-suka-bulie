...
module.exports = function (hexo) {
    hexo.extend.helper.register('favicon', function () {
        ...
        if (favicon.ico) {
            html += htmlTag('link', { rel: 'icon', type: 'image/x-icon', href: url_for(favicon.ico) });
        }
        if (favicon.apple_touch_icon) {
            html += htmlTag('link', { rel: 'apple-touch-icon', sizes: '180x180', href: url_for(favicon.apple_touch_icon) });
        }
        if (favicon.large) {
            html += htmlTag('link', { rel: 'icon', type: 'image/png', sizes: '192x192', href: url_for(favicon.large) });
        }
        if (favicon.medium) {
            html += htmlTag('link', { rel: 'icon', type: 'image/png', sizes: '32x32', href: url_for(favicon.medium) });
        }
        if (favicon.small) {
            html += htmlTag('link', { rel: 'icon', type: 'image/png', sizes: '16x16', href: url_for(favicon.small) });
        }
